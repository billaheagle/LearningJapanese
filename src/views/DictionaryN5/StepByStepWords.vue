<template>
	<div>
		<v-card outlined>
			<v-card-title><h2>{{text.title}} | Step {{text.step}} | ({{this.first}} s/d {{this.last}})</h2></v-card-title>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters({
				dict: 'wordsN5/getDictionary',
				dictLength: 'wordsN5/getDictionaryLength',
			}),
			score() {
			...
			first() {
				return (this.text.step - 1) * 100 + 1
			},
			last() {
				if(this.dictLength < this.text.step * 100) return this.dictLength
				return this.text.step * 100
			}
		},
		methods: {
			start() {
			...
			get100(arr) {
				let newArray = []
				var min = (this.text.step - 1) * 100
				var max = this.text.step * 100
				if(this.dictLength < max) max = this.dictLength
